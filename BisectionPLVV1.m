...
for j = 1:5
    for A = 10:10:100
        ...
        for i = 1:N
            ...
            %load file
            bircenterfilename = sprintf('bir_lfp_v1_center_data_%d.csv', i+(N*(j-1)));
            bircenterdata{i} = importdata(bircenterfilename);
            birsidefilename = sprintf('bir_lfp_v1_right_data_%d.csv', i+(N*(j-1)));
            ...
            %plv
            birisousa{i} = bircenterangle{i} - birsideangle{i};
            birplv{i} = exp(1i*birisousa{i});

            %Bisection Relevant
            %load file
            brcenterfilename = sprintf('br_lfp_v1_center_data_%d.csv', i+(N*(j-1)));
            brcenterdata{i} = importdata(brcenterfilename);
            brsidefilename = sprintf('br_lfp_v1_right_data_%d.csv', i+(N*(j-1)));
            ...
            %plv
            brisousa{i} = brcenterangle{i} - brsideangle{i};
            brplv{i} = exp(1i*brisousa{i});
        end
        ...
        file1name = sprintf('Bisection_IrrelevantResult-bin10Hz-range1Hz-%d.csv', j);
        fileID_1 = fopen(file1name, "a");
        fprintf(fileID_1, "%.4g \n", birplvresult_mean);
        fclose(fileID_1);
        %dlmwrite(file1name, A, birplvresult_mean);
        
        file2name = sprintf('Bisection_RelevantResult-bin10Hz-range1Hz-%d.csv', j);
        fileID_2 = fopen(file2name, "a");
        fprintf(fileID_2, "%.4g \n", brplvresult_mean);
        fclose(fileID_2);
        %dlmwrite(file2name, A, birplvresult_mean);

        file3name = sprintf('Bisection_PLV-freq-%d.csv', j);
