...
#include <math.h>
#include <time.h>
#include "VR_izhikevich_param_V1.h" 
#include "VR_izhikevich_param_VHO.h"
#include "VR_izhikevich_resultfile_V1.h" 
#include "VR_izhikevich_resultfile_VHO.h"
#include "kansuu-izhikevich.h" 
#include "MT.h" 
...
int main(){
	...
	int h, i, j, k, m, n, l;
	double t = 0.0, URN_V1_LGN = 0.0; 
	int train_count = 0; 
	double train_time_start = 0.0, train_time_end = 0.0, train_time_total = 0.0; 
	int train_bisection = 0, train_vernier = 0; 
	int t_bit = 0; 
	int t_lfp = 0; 
    ...
    //V1 Pyr neuron
	double train_V_V1[V1_X_RANGE][V1_Y_RANGE]; 
	double train_U_V1[V1_X_RANGE][V1_Y_RANGE];
	double train_I_V1[V1_X_RANGE][V1_Y_RANGE]; 
	double train_X_V1[V1_X_RANGE][V1_Y_RANGE]; 
	double train_Delta_V1[V1_X_RANGE][V1_Y_RANGE]; 
	double train_S_ex_V1[V1_X_RANGE][V1_Y_RANGE]; 
	double train_S_inh_from_pv_V1[V1_X_RANGE][V1_Y_RANGE];
	double train_S_inh_from_som_V1[V1_X_RANGE][V1_Y_RANGE];
	double train_S_inh_V1[V1_X_RANGE][V1_Y_RANGE];
	double train_LGN_V1[V1_X_RANGE][V1_Y_RANGE]; 
	double train_TOP_V1[V1_X_RANGE][V1_Y_RANGE]; 
	
	double train_LGN_Input_Switch[V1_X_RANGE][V1_Y_RANGE]; 
	double train_V1_synapse[V1_X_RANGE][V1_Y_RANGE];
	double train_X_Input_V1[V1_X_RANGE][V1_Y_RANGE]; 
	double train_Delta_Input_V1[V1_X_RANGE][V1_Y_RANGE];
	//V1 PV neuron
	double train_V1_PV_V[V1_X_RANGE][V1_Y_RANGE];
	double train_V1_PV_U[V1_X_RANGE][V1_Y_RANGE];
	double train_V1_PV_I[V1_X_RANGE][V1_Y_RANGE];
	double train_V1_PV_X[V1_X_RANGE][V1_Y_RANGE];
	double train_V1_PV_Delta[V1_X_RANGE][V1_Y_RANGE];
	double train_V1_PV_S_ex[V1_X_RANGE][V1_Y_RANGE];
	//V1 SOM neuron
	double train_V1_SOM_V[V1_X_RANGE][V1_Y_RANGE];
	double train_V1_SOM_U[V1_X_RANGE][V1_Y_RANGE];
	double train_V1_SOM_I[V1_X_RANGE][V1_Y_RANGE];
	double train_V1_SOM_X[V1_X_RANGE][V1_Y_RANGE];
	double train_V1_SOM_Delta[V1_X_RANGE][V1_Y_RANGE];
	double train_V1_SOM_S_ex[V1_X_RANGE][V1_Y_RANGE];
	double train_V1_SOM_S_inh[V1_X_RANGE][V1_Y_RANGE];
	//V1 Cross correlation
	double BIT_V1[V1_X_RANGE][V1_Y_RANGE]; 
	double BIT_COUNT_V1[V1_X_RANGE][V1_Y_RANGE]; 
	double LINE_BIT_V1[5]; 
	//V1 LFP coherence
	double LFP_V1_synapse[V1_X_RANGE][V1_Y_RANGE];
	double LFP_synapse_V1[5];
	//V1 Tuning curve
	double spike_count_V1[V1_X_RANGE][V1_Y_RANGE];
	...
	double train_BOT_VHO[VHO_X_RANGE][VHO_Y_RANGE];
	double train_TOP_VHO[VHO_X_RANGE][VHO_Y_RANGE]; 

	double train_VHO_synapse[VHO_X_RANGE][VHO_Y_RANGE];
	double train_X_Input_VHO[VHO_X_RANGE][VHO_Y_RANGE]; 
	double train_Delta_Input_VHO[VHO_X_RANGE][VHO_Y_RANGE];
	...
	double train_VHO_SOM_S_inh[VHO_X_RANGE][VHO_Y_RANGE];
	...
	for(i = 0; i < VHO_X_RANGE; i++){
		for(j = 0; j < VHO_Y_RANGE; j++){
			...
			train_X_Input_VHO[i][j] = 0.0; 
			train_Delta_Input_VHO[i][j] = 0.0;
        ...
        for(train_vernier = -2; train_vernier <= 2; train_vernier++){ //Move vernier offset
			...
			for(train_bisection = -2; train_bisection <= 2; train_bisection++){ //Move bisection offset
                ...
                for(t = train_time_start; t <= train_time_end; t = t + dt){
					...
					for(i = 0; i < V1_X_RANGE; i++){
						for(j = 0; j < V1_Y_RANGE; j++){
							...
							if(train_V_V1[i][j] >= th_V1){
                                ...
                                if(train_count == 0){
									if(i == 9 && j == 9){
										fprintf(train_vr_v1_center, "%lf %lf\n", t, train_V_V1[i][j]);
										fprintf(train_vr_X_v1_center, "%lf %lf\n", t, train_X_V1[i][j]);
									}
                                    if(i == 2 && j == 9){
										fprintf(train_vr_v1_left, "%lf %lf\n", t, train_V_V1[i][j]);
										fprintf(train_vr_X_v1_left, "%lf %lf\n", t, train_X_V1[i][j]);
									}
									if(i == 12 && j == 9){
										fprintf(train_vr_v1_right, "%lf %lf\n", t, train_V_V1[i][j]);
										fprintf(train_vr_X_v1_right, "%lf %lf\n", t, train_X_V1[i][j]);
									}
									if(i == 7 && j == 4){
										fprintf(train_vr_v1_top, "%lf %lf\n", t, train_V_V1[i][j]);
										fprintf(train_vr_X_v1_top, "%lf %lf\n", t, train_X_V1[i][j]);
									}
									if(i == 7 && j == 15){
										fprintf(train_vr_v1_bottom, "%lf %lf\n", t, train_V_V1[i][j]);
										fprintf(train_vr_X_v1_bottom, "%lf %lf\n", t, train_X_V1[i][j]);
									}
								}
							...
							if(train_V1_PV_V[i][j] >= th_V1){
                        		...
                        		if(train_count == 0){
									if(i == 9 && j == 9){
									fprintf(train_vr_pv_v1_center, "%lf %lf\n", t, train_V1_PV_V[i][j]);
									fprintf(train_vr_X_pv_v1_center, "%lf %lf\n", t, train_V1_PV_X[i][j]);
									}
								}
							...
							if(train_V1_SOM_V[i][j] >= th_V1){
                        		...
                        		if(train_count == 0){
									if(i == 9 && j == 9){
									fprintf(train_vr_som_v1_center, "%lf %lf\n", t, train_V1_SOM_V[i][j]);
									fprintf(train_vr_X_som_v1_center, "%lf %lf\n", t, train_V1_SOM_X[i][j]);
									}
									if(i == 12 && j == 9){
									fprintf(train_vr_som_v1_right, "%lf %lf\n", t, train_V1_SOM_V[i][j]);
									}
								}
					...
					for(i = 0; i < VHO_X_RANGE; i++){
						for(j = 0; j < VHO_Y_RANGE; j++){
							...
							if(train_V_VHO[i][j] >= th_VHO){
                                ...
                                if(train_count == 0){
									if(i == 9 && j == 1){
										fprintf(train_vr_vho_center, "%lf %lf\n", t, train_V_VHO[i][j]);
										fprintf(train_vr_X_vho_center, "%lf %lf\n", t, train_X_VHO[i][j]);
									}
                                    if(i == 2 && j == 1){
										fprintf(train_vr_vho_left, "%lf %lf\n", t, train_V_VHO[i][j]);
										fprintf(train_vr_X_vho_left, "%lf %lf\n", t, train_X_VHO[i][j]);
									}
									if(i == 12 && j == 1){
										fprintf(train_vr_vho_right, "%lf %lf\n", t, train_V_VHO[i][j]);
										fprintf(train_vr_X_vho_right, "%lf %lf\n", t, train_X_VHO[i][j]);
									}
									if(i == 7 && j == 0){
										fprintf(train_vr_vho_top, "%lf %lf\n", t, train_V_VHO[i][j]);
										fprintf(train_vr_X_vho_top, "%lf %lf\n", t, train_X_VHO[i][j]);
									}
									if(i == 7 && j == 2){
										fprintf(train_vr_vho_bottom, "%lf %lf\n", t, train_V_VHO[i][j]);
										fprintf(train_vr_X_vho_bottom, "%lf %lf\n", t, train_X_VHO[i][j]);
									}
								}
							...
							if(train_VHO_PV_V[i][j] >= th_VHO){
                        		...
                        		if(train_count == 0){
									if(i == 9 && j == 1){
									fprintf(train_vr_pv_vho_center, "%lf %lf\n", t, train_VHO_PV_V[i][j]);
									fprintf(train_vr_X_pv_vho_center, "%lf %lf\n", t, train_VHO_PV_X[i][j]);
									}
								}
							...
							if(train_VHO_SOM_V[i][j] >= th_VHO){
                        		...
                        		if(train_count == 0){
									if(i == 9 && j == 1){
									fprintf(train_vr_som_vho_center, "%lf %lf\n", t, train_VHO_SOM_V[i][j]);
									fprintf(train_vr_X_som_vho_center, "%lf %lf\n", t, train_VHO_SOM_X[i][j]);
									}
								}
					...
					if((t_lfp % T_LFP) == 0){
						if(t > 200.0 && t <= 400.0){
							fprintf(vr_lfp_v1_center_synapse, "%lf\n", LFP_synapse_V1[0] / 4.0);
							fprintf(vr_lfp_v1_left_synapse, "%lf\n", LFP_synapse_V1[1] / 4.0);
							fprintf(vr_lfp_v1_right_synapse, "%lf\n", LFP_synapse_V1[2] / 4.0);
							fprintf(vr_lfp_v1_top_synapse, "%lf\n", LFP_synapse_V1[3] / 4.0);
							fprintf(vr_lfp_v1_bottom_synapse, "%lf\n", LFP_synapse_V1[4] / 4.0);

							fprintf(vr_lfp_vho_center_synapse, "%lf\n", LFP_synapse_VHO[0]);
							fprintf(vr_lfp_vho_left_synapse, "%lf\n", LFP_synapse_VHO[1]);
							fprintf(vr_lfp_vho_right_synapse, "%lf\n", LFP_synapse_VHO[2]);
							fprintf(vr_lfp_vho_top_synapse, "%lf\n", LFP_synapse_VHO[3]);
							fprintf(vr_lfp_vho_bottom_synapse, "%lf\n", LFP_synapse_VHO[4]);
						}
						if(t > 600.0 && t <= 800.0){
							fprintf(vr_lfp_v1_center_synapse, "%lf\n", LFP_synapse_V1[0] / 4.0);
							fprintf(vr_lfp_v1_left_synapse, "%lf\n", LFP_synapse_V1[1] / 4.0);
					...
					for(k = 8; k < 12; k++){
						//V1 PV -> Pyr
						if(train_vernier != 0){
							train_S_inh_from_pv_V1[h][k] = -1 * w_inh_V1_PV_PC_Relevant * train_V1_PV_X[h][h];
							...
							train_S_inh_from_pv_V1[l][k-6] = -1 * w_inh_V1_PV_PC_Relevant * train_V1_PV_X[l][h-5];
							train_S_inh_from_pv_V1[l][k+6] = -1 * w_inh_V1_PV_PC_Relevant * train_V1_PV_X[l][h+6];
						}
						if(train_vernier == 0){
						...
						//V1 SOM -> Pyr
						if(train_vernier != 0){
							train_S_inh_from_som_V1[h][k] = -1 * w_inh_V1_SOM_PC_Relevant * train_V1_SOM_X[h][h];
							...
							train_S_inh_from_som_V1[l][k-6] = -1 * w_inh_V1_SOM_PC_Relevant * train_V1_SOM_X[l][h-5];
							train_S_inh_from_som_V1[l][k+6] = -1 * w_inh_V1_SOM_PC_Relevant * train_V1_SOM_X[l][h+6];
						}
						if(train_vernier == 0){
					...
					//VHO PV -> Pyr
					if(train_vernier != 0){
						train_S_inh_from_pv_VHO[h][1] = -1 * w_inh_VHO_PV_PC_Relevant * train_VHO_PV_X[h][1];
						...
						train_S_inh_from_pv_VHO[l][0] = -1 * w_inh_VHO_PV_PC_Relevant * train_VHO_PV_X[l][0];
						train_S_inh_from_pv_VHO[l][2] = -1 * w_inh_VHO_PV_PC_Relevant * train_VHO_PV_X[l][2];
					}
					if(train_vernier == 0){
					...
					//VHO SOM -> Pyr
					if(train_vernier != 0){
						train_S_inh_from_som_VHO[h][1] = -1 * w_inh_VHO_SOM_PC_Relevant * train_VHO_SOM_X[h][1];
						...
						train_S_inh_from_som_VHO[l][0] = -1 * w_inh_VHO_SOM_PC_Relevant * train_VHO_SOM_X[l][0];
						train_S_inh_from_som_VHO[l][2] = -1 * w_inh_VHO_SOM_PC_Relevant * train_VHO_SOM_X[l][2];
					}
					if(train_vernier == 0){
					...
					if(train_vernier != 0){
						if(fmod(t, Input_T) <= (Input_T / 2)){
							train_V1_SOM_S_inh[h][h] = 0.0;
							train_V1_SOM_S_inh[l][h-5] = -1 * Topdown_SOM;
							train_V1_SOM_S_inh[l][h+6] = -1 * Topdown_SOM;
						}else{
							train_V1_SOM_S_inh[h][h] = -1 * Topdown_SOM;
							train_V1_SOM_S_inh[l][h-5] = 0.0;
							train_V1_SOM_S_inh[l][h+6] = 0.0;
						}
					}
					if(train_vernier == 0){
						train_V1_SOM_S_inh[h][h] = 0.0;
					...
					if(train_vernier != 0){
						if(fmod(t, Input_T) <= (Input_T / 2)){
							train_VHO_SOM_S_inh[h][1] = 0.0;
							train_VHO_SOM_S_inh[l][0] = -1 * Topdown_SOM;
							train_VHO_SOM_S_inh[l][2] = -1 * Topdown_SOM;
						}else{
							train_VHO_SOM_S_inh[h][1] = -1 * Topdown_SOM;
							train_VHO_SOM_S_inh[l][0] = 0.0;
							train_VHO_SOM_S_inh[l][2] = 0.0;
						}
					}
					if(train_vernier == 0){
						train_VHO_SOM_S_inh[h][1] = 0.0;
					...
					for(i = 0; i < V1_X_RANGE; i++){
						for(j = 0; j < V1_Y_RANGE; j++){
							...
							train_V1_SOM_I[i][j] = train_V1_SOM_S_ex[i][j] + train_V1_SOM_S_inh[i][j];

                            if(train_count == 0){
								if(i == 9 && j == 9){
									fprintf(train_vr_Input_v1_center, "%lf %lf\n", t, train_I_V1[i][j]);
                                    fprintf(train_vr_S_ex_v1_center, "%lf %lf\n", t, train_S_ex_V1[i][j]);
									fprintf(train_vr_S_inh_from_pv_v1_center, "%lf %lf\n", t, train_S_inh_from_pv_V1[i][j]);
									fprintf(train_vr_S_inh_from_som_v1_center, "%lf %lf\n", t, train_S_inh_from_som_V1[i][j]);
									fprintf(train_vr_S_v1_center, "%lf %lf\n", t, (train_S_ex_V1[i][j] + train_S_inh_V1[i][j]));
									fprintf(train_vr_Top_v1_center, "%lf %lf\n", t, train_TOP_V1[i][j]);

									fprintf(train_vr_Input_pv_v1_center, "%lf %lf\n", t, train_V1_PV_I[i][j]);
                                    fprintf(train_vr_S_ex_pv_v1_center, "%lf %lf\n", t, train_V1_PV_S_ex[i][j]);

									fprintf(train_vr_Input_som_v1_center, "%lf %lf\n", t, train_V1_SOM_I[i][j]);
                                    fprintf(train_vr_S_ex_som_v1_center, "%lf %lf\n", t, train_V1_SOM_S_ex[i][j]);
									fprintf(train_vr_S_inh_som_v1_center, "%lf %lf\n", t, train_V1_SOM_S_inh[i][j]);
									fprintf(train_vr_S_som_v1_center, "%lf %lf\n", t, (train_V1_SOM_S_ex[i][j] + train_V1_SOM_S_inh[i][j]));
								}
								if(i == 12 && j == 9){								
									fprintf(train_vr_Input_v1_right, "%lf %lf\n", t, train_I_V1[i][j]);
                                    fprintf(train_vr_S_ex_v1_right, "%lf %lf\n", t, train_S_ex_V1[i][j]);
									fprintf(train_vr_S_inh_v1_right, "%lf %lf\n", t, train_S_inh_V1[i][j]);
									fprintf(train_vr_S_v1_right, "%lf %lf\n", t, (train_S_ex_V1[i][j] + train_S_inh_V1[i][j]));

									fprintf(train_vr_Input_pv_v1_right, "%lf %lf\n", t, train_V1_PV_I[i][j]);
                                    fprintf(train_vr_S_ex_pv_v1_right, "%lf %lf\n", t, train_V1_PV_S_ex[i][j]);

									fprintf(train_vr_Input_som_v1_right, "%lf %lf\n", t, train_V1_SOM_I[i][j]);
                                    fprintf(train_vr_S_ex_som_v1_right, "%lf %lf\n", t, train_V1_SOM_S_ex[i][j]);
									fprintf(train_vr_S_inh_som_v1_right, "%lf %lf\n", t, train_V1_SOM_S_inh[i][j]);
								}
								if(i == 2 && j == 9){
									fprintf(train_vr_Input_v1_left, "%lf %lf\n", t, train_I_V1[i][j]);
                                    fprintf(train_vr_S_ex_v1_left, "%lf %lf\n", t, train_S_ex_V1[i][j]);
									fprintf(train_vr_S_inh_v1_left, "%lf %lf\n", t, train_S_inh_V1[i][j]);
									fprintf(train_vr_S_v1_left, "%lf %lf\n", t, (train_S_ex_V1[i][j] + train_S_inh_V1[i][j]));

									fprintf(train_vr_Input_pv_v1_left, "%lf %lf\n", t, train_V1_PV_I[i][j]);
                                    fprintf(train_vr_S_ex_pv_v1_left, "%lf %lf\n", t, train_V1_PV_S_ex[i][j]);

									fprintf(train_vr_Input_som_v1_left, "%lf %lf\n", t, train_V1_SOM_I[i][j]);
                                    fprintf(train_vr_S_ex_som_v1_left, "%lf %lf\n", t, train_V1_SOM_S_ex[i][j]);
									fprintf(train_vr_S_inh_som_v1_left, "%lf %lf\n", t, train_V1_SOM_S_inh[i][j]);
								}
								if(i == 7 && j == 4){
                                    fprintf(train_vr_Input_v1_top, "%lf %lf\n", t, train_I_V1[i][j]);
                                    fprintf(train_vr_S_ex_v1_top, "%lf %lf\n", t, train_S_ex_V1[i][j]);
									fprintf(train_vr_S_inh_v1_top, "%lf %lf\n", t, train_S_inh_V1[i][j]);
									fprintf(train_vr_S_v1_top, "%lf %lf\n", t, (train_S_ex_V1[i][j] + train_S_inh_V1[i][j]));

									fprintf(train_vr_Input_pv_v1_top, "%lf %lf\n", t, train_V1_PV_I[i][j]);
                                    fprintf(train_vr_S_ex_pv_v1_top, "%lf %lf\n", t, train_V1_PV_S_ex[i][j]);

									fprintf(train_vr_Input_som_v1_top, "%lf %lf\n", t, train_V1_SOM_I[i][j]);
                                    fprintf(train_vr_S_ex_som_v1_top, "%lf %lf\n", t, train_V1_SOM_S_ex[i][j]);
									fprintf(train_vr_S_inh_som_v1_top, "%lf %lf\n", t, train_V1_SOM_S_inh[i][j]);
                                }
								if(i == 7 && j == 15){
                                    fprintf(train_vr_Input_v1_bottom, "%lf %lf\n", t, train_I_V1[i][j]);
                                    fprintf(train_vr_S_ex_v1_bottom, "%lf %lf\n", t, train_S_ex_V1[i][j]);
									fprintf(train_vr_S_inh_v1_bottom, "%lf %lf\n", t, train_S_inh_V1[i][j]);
									fprintf(train_vr_S_v1_bottom, "%lf %lf\n", t, (train_S_ex_V1[i][j] + train_S_inh_V1[i][j]));

									fprintf(train_vr_Input_pv_v1_bottom, "%lf %lf\n", t, train_V1_PV_I[i][j]);
                                    fprintf(train_vr_S_ex_pv_v1_bottom, "%lf %lf\n", t, train_V1_PV_S_ex[i][j]);

									fprintf(train_vr_Input_som_v1_bottom, "%lf %lf\n", t, train_V1_SOM_I[i][j]);
                                    fprintf(train_vr_S_ex_som_v1_bottom, "%lf %lf\n", t, train_V1_SOM_S_ex[i][j]);
									fprintf(train_vr_S_inh_som_v1_bottom, "%lf %lf\n", t, train_V1_SOM_S_inh[i][j]);
                                }
							}
                    ...
                    /*End to V1 memvrane update*/

					/*Start to VHO memvrane update*/
					for(i = 0; i < VHO_X_RANGE; i++){
						for(j = 0; j < VHO_Y_RANGE; j++){
							...
							train_VHO_SOM_I[i][j] = train_VHO_SOM_S_ex[i][j] + train_VHO_SOM_S_inh[i][j];

                            if(train_count == 0){
								if(i == 9 && j == 1){
									fprintf(train_vr_Input_vho_center, "%lf %lf\n", t, train_I_VHO[i][j]);
                                    fprintf(train_vr_S_ex_vho_center, "%lf %lf\n", t, train_S_ex_VHO[i][j]);
									fprintf(train_vr_S_inh_from_pv_vho_center, "%lf %lf\n", t, train_S_inh_from_pv_VHO[i][j]);
									fprintf(train_vr_S_inh_from_som_vho_center, "%lf %lf\n", t, train_S_inh_from_som_VHO[i][j]);
									fprintf(train_vr_Bot_vho_center, "%lf %lf\n", t, train_BOT_VHO[i][j]);
									fprintf(train_vr_Top_vho_center, "%lf %lf\n", t, train_TOP_VHO[i][j]);
									fprintf(train_vr_S_vho_center, "%lf %lf\n", t, (train_S_ex_VHO[i][j] + train_S_inh_VHO[i][j]));

									fprintf(train_vr_Input_pv_vho_center, "%lf %lf\n", t, train_VHO_PV_I[i][j]);
                                    fprintf(train_vr_S_ex_pv_vho_center, "%lf %lf\n", t, train_VHO_PV_S_ex[i][j]);

									fprintf(train_vr_Input_som_vho_center, "%lf %lf\n", t, train_VHO_SOM_I[i][j]);
                                    fprintf(train_vr_S_ex_som_vho_center, "%lf %lf\n", t, train_VHO_SOM_S_ex[i][j]);
									fprintf(train_vr_S_inh_som_vho_center, "%lf %lf\n", t, train_VHO_SOM_S_inh[i][j]);
									fprintf(train_vr_S_som_vho_center, "%lf %lf\n", t, (train_VHO_SOM_S_ex[i][j] + train_VHO_SOM_S_inh[i][j]));
								}
								if(i == 12 && j == 1){								
									fprintf(train_vr_Input_vho_right, "%lf %lf\n", t, train_I_VHO[i][j]);
                                    fprintf(train_vr_S_ex_vho_right, "%lf %lf\n", t, train_S_ex_VHO[i][j]);
									fprintf(train_vr_S_inh_vho_right, "%lf %lf\n", t, train_S_inh_VHO[i][j]);
									fprintf(train_vr_Bot_vho_right, "%lf %lf\n", t, train_BOT_VHO[i][j]);
									fprintf(train_vr_S_vho_right, "%lf %lf\n", t, (train_S_ex_VHO[i][j] + train_S_inh_VHO[i][j]));

									fprintf(train_vr_Input_pv_vho_right, "%lf %lf\n", t, train_VHO_PV_I[i][j]);
                                    fprintf(train_vr_S_ex_pv_vho_right, "%lf %lf\n", t, train_VHO_PV_S_ex[i][j]);
								}
								if(i == 2 && j == 1){
									fprintf(train_vr_Input_vho_left, "%lf %lf\n", t, train_I_VHO[i][j]);
                                    fprintf(train_vr_S_ex_vho_left, "%lf %lf\n", t, train_S_ex_VHO[i][j]);
									fprintf(train_vr_S_inh_vho_left, "%lf %lf\n", t, train_S_inh_VHO[i][j]);
									fprintf(train_vr_Bot_vho_left, "%lf %lf\n", t, train_BOT_VHO[i][j]);
									fprintf(train_vr_S_vho_left, "%lf %lf\n", t, (train_S_ex_VHO[i][j] + train_S_inh_VHO[i][j]));

									fprintf(train_vr_Input_pv_vho_left, "%lf %lf\n", t, train_VHO_PV_I[i][j]);
                                    fprintf(train_vr_S_ex_pv_vho_left, "%lf %lf\n", t, train_VHO_PV_S_ex[i][j]);

									fprintf(train_vr_Input_som_vho_left, "%lf %lf\n", t, train_VHO_SOM_I[i][j]);
                                    fprintf(train_vr_S_ex_som_vho_left, "%lf %lf\n", t, train_VHO_SOM_S_ex[i][j]);
									fprintf(train_vr_S_inh_som_vho_left, "%lf %lf\n", t, train_VHO_SOM_S_inh[i][j]);
								}
								if(i == 7 && j == 0){
                                    fprintf(train_vr_Input_vho_top, "%lf %lf\n", t, train_I_VHO[i][j]);
                                    fprintf(train_vr_S_ex_vho_top, "%lf %lf\n", t, train_S_ex_VHO[i][j]);
									fprintf(train_vr_S_inh_vho_top, "%lf %lf\n", t, train_S_inh_VHO[i][j]);
									fprintf(train_vr_Bot_vho_top, "%lf %lf\n", t, train_BOT_VHO[i][j]);
									fprintf(train_vr_S_vho_top, "%lf %lf\n", t, (train_S_ex_VHO[i][j] + train_S_inh_VHO[i][j]));

									fprintf(train_vr_Input_pv_vho_top, "%lf %lf\n", t, train_VHO_PV_I[i][j]);
                                    fprintf(train_vr_S_ex_pv_vho_top, "%lf %lf\n", t, train_VHO_PV_S_ex[i][j]);

									fprintf(train_vr_Input_som_vho_top, "%lf %lf\n", t, train_VHO_SOM_I[i][j]);
                                    fprintf(train_vr_S_ex_som_vho_top, "%lf %lf\n", t, train_VHO_SOM_S_ex[i][j]);
									fprintf(train_vr_S_inh_som_vho_top, "%lf %lf\n", t, train_VHO_SOM_S_inh[i][j]);
                                }
								if(i == 7 && j == 2){
                                    fprintf(train_vr_Input_vho_bottom, "%lf %lf\n", t, train_I_VHO[i][j]);
                                    fprintf(train_vr_S_ex_vho_bottom, "%lf %lf\n", t, train_S_ex_VHO[i][j]);
									fprintf(train_vr_S_inh_vho_bottom, "%lf %lf\n", t, train_S_inh_VHO[i][j]);
									fprintf(train_vr_Bot_vho_bottom, "%lf %lf\n", t, train_BOT_VHO[i][j]);
									fprintf(train_vr_S_vho_bottom, "%lf %lf\n", t, (train_S_ex_VHO[i][j] + train_S_inh_VHO[i][j]));

									fprintf(train_vr_Input_pv_vho_bottom, "%lf %lf\n", t, train_VHO_PV_I[i][j]);
                                    fprintf(train_vr_S_ex_pv_vho_bottom, "%lf %lf\n", t, train_VHO_PV_S_ex[i][j]);

									fprintf(train_vr_Input_som_vho_bottom, "%lf %lf\n", t, train_VHO_SOM_I[i][j]);
                                    fprintf(train_vr_S_ex_som_vho_bottom, "%lf %lf\n", t, train_VHO_SOM_S_ex[i][j]);
									fprintf(train_vr_S_inh_som_vho_bottom, "%lf %lf\n", t, train_VHO_SOM_S_inh[i][j]);
                                }
							}
			...
			if(train_count == 0){
				fprintf(vr_tuning_VHO_center, "center %d %lf\n", train_vernier, pattern_VHO_center);
				fprintf(vr_tuning_VHO_left, "left %d %lf\n", train_vernier, pattern_VHO_left);
				fprintf(vr_tuning_VHO_right, "right %d %lf\n", train_vernier, pattern_VHO_right);
				fprintf(vr_tuning_VHO_top, "left %d %lf\n", train_vernier, pattern_VHO_top);
				fprintf(vr_tuning_VHO_bottom, "right %d %lf\n", train_vernier, pattern_VHO_bottom);
				if(train_vernier == -2){
					fprintf(vr_tuning_VHO, "top %lf\ncenter %lf\nbottom %lf\n", pattern_VHO_top, pattern_VHO_center, pattern_VHO_bottom);
				}
			}
